...
export class PacketComponent implements OnInit {
  ...
  page:number=1;
  pageSize:number=10;
  item: PacketModel=<PacketModel>{};
  search: string = "";
  totalRecords:number=0;
  selectedItems: PacketModel[];
  submitted: boolean;
...
openNew() {
  this.router.navigate(['/master/CREATE_PACKET_MASTER']);
}
...
editProduct(data: PacketModel) {
  this.router.navigate(['/master/CREATE_PACKET_MASTER']);
 
}

deleteProduct(data: PacketModel) {
  this.confirmationService.confirm({
