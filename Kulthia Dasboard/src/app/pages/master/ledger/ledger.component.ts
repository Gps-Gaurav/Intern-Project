...
export class LedgerComponent implements OnInit {
  ...
  page:number=1;
  pageSize:number=10;
  item: LedgerModel=<LedgerModel>{};
  search: string = "";
  totalRecords:number=0;
  selectedItems: LedgerModel[];
  submitted: boolean;
...
deleteSelectedProducts() {
  this.confirmationService.confirm({
      ...
      accept: () => {
        ...
        requestModel.table_name="LedgerMast";
...
editProduct(data: LedgerModel) {
  this.router.navigate(['/master/CREATE_LEDGER_MASTER', { id: data.SlNo }]);
  // this.item = {...data};
  // this.productDialog = true;
}

deleteProduct(data: LedgerModel) {
  this.confirmationService.confirm({
      message: 'Are you sure you want to delete ' + data.Location + '?',
      header: 'Confirm',
      icon: 'pi pi-exclamation-triangle',
      accept: () => {
        let requestModel = <DeleteModel>{};
