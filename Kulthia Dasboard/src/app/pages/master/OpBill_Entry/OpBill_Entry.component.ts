...
@Component({
  ...
  templateUrl: './OpBill_Entry.component.html',
  styleUrls: ['./OpBill_Entry.component.scss'],
  providers: [MessageService,ConfirmationService]

})
export class OpBILL_ENTRYComponent implements OnInit {
  breadCrumbItems: Array<{}>;
  productDialog: boolean;
  Delete:"Delete";
  items: OpBill_EntryModel[];
  page:number=1;
  pageSize:number=10;
  item: OpBill_EntryModel=<OpBill_EntryModel>{};
  search: string = "";
  totalRecords:number=0;
  selectedItems: OpBill_EntryModel[];
  submitted: boolean;
...
openNew() {
  this.router.navigate(['/master/CREATE_OpBILL_ENTRY_MASTER']);
}
...
deleteSelectedProducts() {
  this.confirmationService.confirm({
      ...
      accept: () => {
        ...
        requestModel.table_name="OpBill_EntryMast";
...
editProduct(data: OpBill_EntryModel) {
  this.router.navigate(['/master/CREATE_OpBILL_ENTRY_MASTER', { id: data.SlNo }]);
  // this.item = {...data};
  // this.productDialog = true;
}

deleteProduct(data: OpBill_EntryModel) {
  this.confirmationService.confirm({
      message: 'Are you sure you want to delete ' + data.OpBill_Entry + '?',
      header: 'Confirm',
      icon: 'pi pi-exclamation-triangle',
      accept: () => {
        let requestModel = <DeleteModel>{};
        requestModel.table_name="OpStockMast";
        requestModel.column_name="SlNo";
