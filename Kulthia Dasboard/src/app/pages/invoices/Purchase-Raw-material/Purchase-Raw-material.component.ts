...
@Component({
  ...
  templateUrl: './Purchase-Raw-material.component.html',
  styleUrls: ['./Purchase-Raw-material.component.scss'],
  providers: [MessageService,ConfirmationService]

})
export class PurchaseRawComponent implements OnInit {
...
public readonly ImgUrl = environment.APIUrl;
  ...
  ngOnInit(): void {
   ...
   private _fetchData() { 
    ...
    requestModel.spName="sp_admin_Get_PurchaseRawMast";
...
openNew() {
  this.router.navigate(['/invoices/purchase-raw-material-add']);
}
...
deleteSelectedProducts() {
  this.confirmationService.confirm({
    ...
    accept: () => {
      ...
      requestModel.table_name="PurHead";
      requestModel.column_name="PurSlNo";
      requestModel.column_value=this.selectedItems.map(a=>a.PurSlNo).toString();
...
editProduct(data: InseretFinishesGoodsModel) {
  this.router.navigate(['/invoices/purchase-raw-material-add', { id: data.PurSlNo }]);
 
}

deleteProduct(data: InseretFinishesGoodsModel) {
  this.confirmationService.confirm({
      message: 'Are you sure you want to delete ' + data.PurSlNo+ '?',
      header: 'Confirm',
      icon: 'pi pi-exclamation-triangle',
      accept: () => {
        let requestModel = <DeleteModel>{};
        requestModel.table_name="PurHead";
        requestModel.column_name="PurSlNo";
        requestModel.column_value=data.PurSlNo.toString();
        this.service.DeleteApiJson(requestModel).subscribe(response => {
        console.log("delete",response);
        this.items = this.items.filter(val => val.PurSlNo !== data.PurSlNo);
