...
@Component({
  ...
  templateUrl: './insert_TBL_STOCK_TRANSFER.component.html',
  styleUrls: ['./insert_TBL_STOCK_TRANSFER.component.scss'],
 ...
 */
export class InsertStockTransferComponent implements OnInit, AfterViewInit, OnDestroy {
 ...
 totalqty=0;
  ...
  ngOnInit() {
    ...
    this.formData = this.formBuilder.group({
      ...
      IssuesDt: [this.datepipe.transform(new Date(), 'yyyy-MM-dd'),],
 ...
 selectEvent(item) {
  ...
  validSubmit() {
    ...
    if (this.formData.valid) {
      let request: StockTrasferModel  = <StockTrasferModel >{};

     request.TransNo = this.formData.get('TransNo').value;///
     request.PartyID = this.formData.get('PartyID').value;//
    request.LocationFrom = this.formData.get('LocationFrom').value;//
      request.LocationTo = this.formData.get('LocationTo').value;//
      request.IssuesDt=this.formData.get('IssuesDt').value;//
      request.TransDt = this.formData.get('TransDt').value;//
      request.TotPcs = this.formData.get('TotPcs').value;//
      ...
      let jsonData: StockTrasferModel [] = [];
      ...
      this.service.PostApiJson(postrequest).subscribe(Response => {
        if (Response.document.statusMessage === "success") {
          ...
          this.router.navigate(['/invoices/Stock_Transfer']);
        }
        else {
    ...
    this.service.GetApiJson(requestModel).subscribe(response => {
      ...
      if (this.SaleData.length) {
       ...
       this.LocationID=data['LocationID'];
        ...
        this.formData.controls['LocationFrom'].setValue(Number(data['LocationFrom']));
        this.formData.controls['LocationTo'].setValue(data['LocationTo']);
        this.formData.controls['IssuesDt'].setValue(data['IssuesDt']);
...
Close(){
  this.router.navigate(['/invoices/SaleInvoice']);
}
