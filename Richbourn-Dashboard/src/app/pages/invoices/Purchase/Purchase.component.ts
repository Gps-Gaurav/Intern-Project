...
export class PurchaseComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.righmenu = [
      ...
      {label: 'Purchase Voucher', icon: 'pi pi-fw pi-file', command: () => this.openVoucher(this.selectedProduct)},
      {label: 'Delete', icon: 'pi pi-fw pi-trash', command: () => this.deleteProduct(this.selectedProduct)}
  ];
    this. _fetchData();
   }
   viewProduct(product: PurchaseModel) {
...
openNew() {
  this.router.navigate(['/invoices/insert_TBL_PURCHASE']);
}
...
editProduct(data: PurchaseModel) {
  
  this.router.navigate(['/invoices/insert_TBL_PURCHASE', { id: data.SlNo }]);
  // this.item = {...data};
  // this.productDialog = true;
}
openPdf(data: PurchaseModel) {
  this.router.navigate(['/invoices/Purchase_Detail_Invoices', { id: data.SlNo }]);
 
}
openVoucher(data: PurchaseModel){
  this.router.navigate(['/invoices/Purchase-Voucher', { id: data.SlNo }]);
  
}
deleteProduct(data: PurchaseModel) {
  this.confirmationService.confirm({
