...
import { Purchase_ReturnModel } from 'src/app/core/models/PurchaseReturn.model';
...
export class PurchaseReturnComponent implements OnInit {
  ...
  Delete:"Delete";
  items: Purchase_ReturnModel[];
  page:number=1;
  pageSize:number=10;
  item: Purchase_ReturnModel=<Purchase_ReturnModel>{};
  search: string = "";
  totalRecords:number=0;
  selectedItems: Purchase_ReturnModel[];
  submitted: boolean;
  searchChanged: Subject<string> = new Subject<string>();
  term:any;
  selectedProduct: Purchase_ReturnModel;
  righmenu: MenuItem[];
cm: any;
  ...
  ngOnInit(): void {
   ...
   viewProduct(product: Purchase_ReturnModel) {
    this.messageService.add({severity: 'info', summary: 'Product Selected', detail: product.InvNo });
   }
  
   private _fetchData() { 
    let requestModel = <RequestModel>{};
    requestModel.spName="sp_admin_GetPurchaesReturnMast";
...
openNew() {
  this.router.navigate(['/invoices/insert_TBL_PURCHASE_RETURN']);
}
...
deleteSelectedProducts() {
  this.confirmationService.confirm({
      ...
      accept: () => {
        ...
        requestModel.table_name="ProdPurchaseReturnHead";
...
editProduct(data:Purchase_ReturnModel) {
  this.router.navigate(['/invoices/insert_TBL_PURCHASE_RETURN', { id: data.SlNo }]);
 
}
openPdf(data: Purchase_ReturnModel) {
 // this.router.navigate(['/invoices/Sale_Detail_Invoices', { id: data.SlNo }]);
  this.service.getPdf(data.SlNo )
  .pipe(take(1))
  .subscribe((response) => {
      ...
      const fileName =data.SlNo +"_report.pdf";
      downloadLink.download = fileName;
      downloadLink.click();
  });
}
deleteProduct(data: Purchase_ReturnModel) {
  this.confirmationService.confirm({
      ...
      accept: () => {
        ...
        this.item =<Purchase_ReturnModel>{};
