...
import { SaleRawModel } from 'src/app/core/models/SaleRaw.model';
...
@Component({
  ...
  templateUrl: './insert_TBL_SALE_RAW.component.html',
  styleUrls: ['./insert_TBL_SALE_RAW.component.scss'],
 ...
 */
export class insert_TBL_SALE_RAWComponent implements OnInit, AfterViewInit, OnDestroy {
 ...
 SaleRawData: SaleRawModel[] = [];
  ...
  validSubmit() {
    ...
    if (this.formData.valid) {
      let request:SaleRawModel = <SaleRawModel>{};

      request.SaleNo = this.formData.get('SaleNo').value;//
      request.SaleDate = this.formData.get('SaleDate').value;//
      ...
      let jsonData: SaleRawModel[] = [];
      jsonData.push(request);
      let postrequest: RequestModel = <RequestModel>{};

      postrequest.spName = "sp_ui_admin_add_SaleRawMast_json";
  ...
  private _fetchData() {
    ...
    requestModel.spName = "sp_admin_GetSaleRawMast_SlNo";
    ...
    this.service.GetApiJson(requestModel).subscribe(response => {
      this.SaleRawData = JSON.parse(response.items);
      if (this.SaleRawData.length) {
        let data = this.SaleRawData[0];
        ...
        this.formData.controls['SaleDate'].setValue(data['SaleDate']);
