...
import { TBL_ITEM_MASTERModel } from './TBL_ITEM_MASTER.model';
import Swal from 'sweetalert2';
import { ToastrService } from 'ngx-toastr';
import { Router } from '@angular/router';
import { debounceTime, distinctUntilChanged } from 'rxjs/operators';
@Component({
  selector: 'app-TBL_ITEM_MASTER',
  templateUrl: './TBL_ITEM_MASTER.component.html',
  styleUrls: ['./TBL_ITEM_MASTER.component.scss']
})

/**
 * Ecomerce Customers component
 */
export class TBL_ITEM_MASTERComponent implements OnInit {

  // bread crumb items
  breadCrumbItems: Array<{}>;
  formData: FormGroup;
  submitted = false;
  tableData: TBL_ITEM_MASTERModel[];
  ...
  constructor(private modalService: NgbModal,private router: Router, private formBuilder: FormBuilder,private service:ResourceService<TBL_ITEM_MASTERModel>,private toastr: ToastrService) { 
...
ENTRY_BY: ['', [Validators.required]],
   ...
   */
   ...
   private _fetchData() {
    ...
    this.requestModel.url = "TBL_ITEM_MASTER?PageNo=" + this.page + "&PageSize=" + this.pageSize + "&Search=" + this.search;
   ...
   */
   openModal(content: any) {
    this.router.navigate(['/master/insert_TBL_ITEM_MASTER']);
  }
  edit(data:Number){
    this.router.navigate(['/master/insert_TBL_ITEM_MASTER', { id: data }]);
