...
import { TBL_STATIONARY_MASTERModel } from './TBL_STATIONARY_MASTER.model';
import Swal from 'sweetalert2';
import { ToastrService } from 'ngx-toastr';
import { Router } from '@angular/router';
@Component({
  selector: 'app-TBL_STATIONARY_MASTER',
  templateUrl: './TBL_STATIONARY_MASTER.component.html',
  styleUrls: ['./TBL_STATIONARY_MASTER.component.scss']
})

/**
 * Ecomerce Customers component
 */
export class TBL_STATIONARY_MASTERComponent implements OnInit {

  // bread crumb items
  breadCrumbItems: Array<{}>;
  formData: FormGroup;
  submitted = false;
  tableData: TBL_STATIONARY_MASTERModel[];
  ...
  constructor(private modalService: NgbModal,private router: Router, private formBuilder: FormBuilder,private service:ResourceService<TBL_STATIONARY_MASTERModel>,private toastr: ToastrService) { }

  ngOnInit() {
    this.breadCrumbItems = [{ label: 'Ecommerce' }, { label: 'TBL_STATIONARY_MASTER', active: true }];
...
TO_RANGE: ['', [Validators.required]],
   ...
   */
    /*
   private _fetchData() {
    this.requestModel.url = "SP/sp_get_list_TBL_STATIONARY_MASTER?PageNo="+this.page+"&PageSize="+this.pageSize;
    ...
    */
   private _fetchData() {
    this.requestModel.url = "TBL_STATIONARY_MASTER";
   ...
   */
   openModal(content: any) {
    this.router.navigate(['/master/insert_TBL_STATIONARY_MASTER']);
  }
  edit(data:TBL_STATIONARY_MASTERModel,content: any){
...
request.TO_RANGE = this.formData.get('TO_RANGE').value;
  ...
  confirm(id) {
    ...
    Swal.fire({
      ...
      if (result.value) {
        this.service.delete(id,"TBL_STATIONARY_MASTER/id?id=").subscribe(response => {
