...
import { TBL_FINANCE_BANK_MASTERModel } from './TBL_FINANCE_BANK_MASTER.model';
import Swal from 'sweetalert2';
import { ToastrService } from 'ngx-toastr';
import { Router } from '@angular/router';
@Component({
  selector: 'app-TBL_FINANCE_BANK_MASTER',
  templateUrl: './TBL_FINANCE_BANK_MASTER.component.html',
  styleUrls: ['./TBL_FINANCE_BANK_MASTER.component.scss']
})

/**
 * Ecomerce Customers component
 */
export class TBL_FINANCE_BANK_MASTERComponent implements OnInit {

  // bread crumb items
  breadCrumbItems: Array<{}>;
  formData: FormGroup;
  submitted = false;
  tableData: TBL_FINANCE_BANK_MASTERModel[];
  ...
  constructor(private modalService: NgbModal,private router: Router, private formBuilder: FormBuilder,private service:ResourceService<TBL_FINANCE_BANK_MASTERModel>,private toastr: ToastrService) { }
...
ACC_NO: ['', [Validators.required]],
   ...
   */
    /*
   private _fetchData() {
    this.requestModel.url = "SP/sp_get_list_TBL_FINANCE_BANK_MASTER?PageNo="+this.page+"&PageSize="+this.pageSize;
    ...
    */
   private _fetchData() {
    this.requestModel.url = "TBL_FINANCE_BANK_MASTER";
   ...
   */
   openModal(content: any) {
    this.router.navigate(['/master/insert_TBL_FINANCE_BANK_MASTER']);
  }
  edit(data:number){
    this.router.navigate(['/master/insert_TBL_FINANCE_BANK_MASTER', { id: data }]);
  }
  saveData() {
...
request.ACC_NO = this.formData.get('ACC_NO').value;
  ...
  confirm(id) {
    ...
    Swal.fire({
      ...
      if (result.value) {
        this.service.delete(id,"TBL_FINANCE_BANK_MASTER/id?id=").subscribe(response => {
