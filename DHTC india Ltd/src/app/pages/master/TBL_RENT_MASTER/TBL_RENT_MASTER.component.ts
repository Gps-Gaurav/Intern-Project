...
import { TBL_RENT_MASTERModel } from './TBL_RENT_MASTER.model';
import Swal from 'sweetalert2';
import { ToastrService } from 'ngx-toastr';
import { Router } from '@angular/router';
import { debounceTime, distinctUntilChanged } from 'rxjs/operators';
@Component({
  selector: 'app-TBL_RENT_MASTER',
  templateUrl: './TBL_RENT_MASTER.component.html',
  styleUrls: ['./TBL_RENT_MASTER.component.scss']
})

/**
 * Ecomerce Customers component
 */
export class TBL_RENT_MASTERComponent implements OnInit {

  // bread crumb items
  breadCrumbItems: Array<{}>;
  formData: FormGroup;
  submitted = false;
  tableData: TBL_RENT_MASTERModel[];
  ...
  constructor(private modalService: NgbModal,private router: Router, private formBuilder: FormBuilder,private service:ResourceService<TBL_RENT_MASTERModel>,private toastr: ToastrService) { 
...
TO_DATE: ['', [Validators.required]],
   ...
   */
    /*
   private _fetchData() {
    this.requestModel.url = "SP/sp_get_list_TBL_RENT_MASTER?PageNo="+this.page+"&PageSize="+this.pageSize;
    ...
    */
   private _fetchData() {
    // this.requestModel.url = "TBL_RENT_MASTER";
    ...
    this.requestModel.url = "TBL_RENT_MASTER?PageNo=" + this.page + "&PageSize=" + this.pageSize + "&Search=" + this.search;
   ...
   */
   openModal(content: any) {
    this.router.navigate(['/master/insert_TBL_RENT_MASTER']);
  }
  edit(data:Number){
    this.router.navigate(['/master/insert_TBL_RENT_MASTER', { id: data }]);
...
request.RENT = this.formData.get('RENT').value;
...
request.TO_DATE = this.formData.get('TO_DATE').value;
  ...
  confirm(id) {
    ...
    Swal.fire({
      ...
      if (result.value) {
        this.service.delete(id,"TBL_RENT_MASTER/id?id=").subscribe(response => {
