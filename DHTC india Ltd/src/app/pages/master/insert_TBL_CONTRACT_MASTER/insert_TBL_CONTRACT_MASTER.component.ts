...
import { TBL_CONTRACT_MASTERModel } from '../TBL_CONTRACT_MASTER/TBL_CONTRACT_MASTER.model';
@Component({
  selector: 'app-insert-TBL_CONTRACT_MASTER',
  templateUrl: './insert-TBL_CONTRACT_MASTER.component.html',
  styleUrls: ['./insert-TBL_CONTRACT_MASTER.component.scss']
})

/**
 * Ecomerce Customers component
 */
export class InsertTBL_CONTRACT_MASTERComponent implements OnInit {

 constructor(public formBuilder: FormBuilder, private service:ResourceService<TBL_CONTRACT_MASTERModel>,
 ...
 id:Number=0;
  ...
  ngOnInit() {
    this.breadCrumbItems = [{ label: 'Master' }, { label: 'TBL_CONTRACT_MASTER', active: true }];
...
request.REMARKS = this.formData.get('REMARKS').value;
     ...
     // formData.append('id', this.id.toString());
      ...
      this.service.add(request,"TBL_CONTRACT_MASTER").subscribe(response => {
       
        console.log("response",response);
        if(response.document>0){
          this.toastr.success( 'Product Inserted Successfully!','Success!');
          this.router.navigate(['/admin/TBL_CONTRACT_MASTER']);
 ...
 private _fetchData() {
  ...
  this.service.get("TBL_CONTRACT_MASTER/id?id="+this.id).subscribe(response => {
