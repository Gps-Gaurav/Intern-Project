...
import { DeleteModel } from '../data/model/delete.model';
import { ResponseData, ResponseToken } from '../data/model/responsedata.model';
import { DocumentModel, SingleDocumentModel } from '../data/model/document.model';
...
export  class ResourceService<T> {
  ...
  PostApiJson(resource: any): Observable<ResponseData<T>> {
  ...
  GetFilterApiJson(resource: RequestModel): Observable<ResponseData<T>> {
    ...
    return this.httpClient.post<ResponseData<T>>(this.APIUrl+resource.url,resource)
      .pipe(
        catchError(this.handleError)
      );
  }
 ...
 getList(page: number, count: number,path:String): Observable<ResponseData<DocumentModel<T>>> {
  ...
  getPdf(id):Observable<HttpResponse<Blob>> {

    const httpOptions = {
      responseType: 'blob' as 'json'
    };
    return this.httpClient.get<Blob>(`${this.APIUrl}Report/SaleReport?id=${id}`, { observe: 'response', responseType: 'blob' as 'json'});
    //return this.httpClient.get(`${this.APIUrl}/Report/SaleReport?id=${id}`, httpOptions);
  }
