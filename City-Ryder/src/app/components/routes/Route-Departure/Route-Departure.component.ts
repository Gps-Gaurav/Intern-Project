...
import { RouteDepartureModel } from 'src/app/shared/data/model/RouteDeparture.model';
...
@Component({
  ...
  templateUrl: './Route-Departure.component.html',
  styleUrls: ['./Route-Departure.component.scss'],
  providers: [MessageService,ConfirmationService],
  encapsulation:ViewEncapsulation.None
})
export class RouteDepartureComponent implements OnInit {

  breadCrumbItems: Array<{}>;
  productDialog: boolean;
  Delete:"Delete";
  items: RouteDepartureModel[];
  page:number=1;
  pageSize:number=10;
  item: RouteDepartureModel=<RouteDepartureModel>{};
  search: string = "";
  filter: string = "";
  totalRecords:number=0;
  selectedItems:RouteDepartureModel[];
  submitted: boolean;
  formData: any;
  sizes: any[];
  submit: boolean;
...
editProduct(data:RouteDepartureModel) {
...
deleteProduct(data: RouteDepartureModel) {
  this.confirmationService.confirm({
...
validSubmit() {
  ...
  if (this.formData.valid) {
    let request: RouteDepartureModel = <RouteDepartureModel>{};
